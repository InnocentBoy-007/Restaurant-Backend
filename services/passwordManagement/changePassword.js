...
class ChangePassword {
    async AdminChangePassword(req, res) {
        ...
        if (!passwords || typeof passwords !== 'object') return res.status(400).json({ message: "The request body is invalid or is not an object! - backend!" });
        ...
        try {
            ...
            if (!isValidCurrentPassword) {
                return res.status(409).json({ message: "Incorrect current password! - backend" });
            } else {
                // if there ins't anything wrong with the new password, then hash the password and then save it
